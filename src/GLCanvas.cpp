#include <CoreMath/BasicMath.hpp>
#include <SCV/Point.h>
...
std::vector<CoreMath::Vector4>::iterator GLCanvas::collisionTest(CoreMath::Vector4 p)
{
...
CoreMath::Scalar GLCanvas::ccw(CoreMath::Vector4 v0, CoreMath::Vector4 v1, CoreMath::Vector4 v2)
{
  return ((v1[0] - v0[0]) * (v2[1] - v0[1])) - ((v2[0] - v0[0]) * (v1[1] - v0[1]));
}

std::vector<int> GLCanvas::giftWrap(std::vector<CoreMath::Vector4> points)
{
  ...
  if(points.size() > 2)
  {
    CoreMath::Scalar y = (CoreMath::Scalar) getHeight();
    unsigned int idx = -1;
    ...
    CoreMath::Vector4 pivotPoint(-1, 0);
    do
    {
      idx = points.size() + 1;
      CoreMath::Scalar smlAngle = 2 * CoreMath::PI;

      for(i = 0; i < points.size(); i++)
      {
        CoreMath::Vector4 currPoint = points[i] - points[chPointsIdx[chPointsIdx.size() - 1]];
        currPoint.normalize();
        CoreMath::Scalar angle = CoreMath::ArcCosine(pivotPoint * currPoint);
        ...
        if(angle < smlAngle && angle != 0.0 && angle != CoreMath::PI && angle != 2 * CoreMath::PI)
        {
          smlAngle = angle;
          idx = i;
        }
...
std::vector<int> GLCanvas::grahamScan(std::vector<CoreMath::Vector4> points)
{
  std::vector<int> chPointsIdx;
  std::vector<int> pointsIdx;
  std::vector<CoreMath::Vector4> sortedPoints = points;
  unsigned int i;
  ...
  if(points.size() > 2)
  {
    ...
    CoreMath::Vector4 currPoint(1, 0);
    idx = -1;
    for(i = 1; i < sortedPoints.size(); i++)
    {
      CoreMath::Scalar smlAngle = 2 * CoreMath::PI;
      idx = -1;
      for(unsigned int j = i; j < sortedPoints.size(); j++)
      {
	CoreMath::Vector4 auxVec = sortedPoints[j] - sortedPoints[0];
	auxVec.normalize();
        CoreMath::Scalar jAng = CoreMath::ArcCosine(currPoint * auxVec);
        if(jAng < smlAngle)
        {
          smlAngle = jAng;
          idx = j;
    ...
    //Here we calculate the area of the triangle formed by each of the three points considered.
    //If the area is <= 0, then we remove the middle point from the convex hull.
    i = 2;
    while(i < pointsIdx.size())
    {
      CoreMath::Scalar area = ccw(points[*(chPointsIdx.end() - 2)], points[*(chPointsIdx.end() - 1)], points[pointsIdx[i]]);
      if(area < 0)
      {
...
void GLCanvas::render()
{
  ...
  for(unsigned int i = 0; i < m_vEdges.size(); i++)
    m_vEdges[i].render();
}
...
void GLCanvas::onMouseClick(const scv::MouseEvent &evt)
{
  ...
  {
    auto it = collisionTest(CoreMath::Vector4((CoreMath::Scalar) evt.getPosition().x, (CoreMath::Scalar) evt.getPosition().y));
    if(it == m_vPoints.end())
      m_vPoints.push_back(CoreMath::Vector4((CoreMath::Scalar) evt.getPosition().x, (CoreMath::Scalar) evt.getPosition().y));
  }
  ...
  if(evt.getButton() == evt.LEFT)
  {
    if(m_bMouseHeld == false)
    {
      if(evt.getButton() == evt.LEFT)
      {
        ...
        if(it != m_vPoints.end())
        {
          *it = CoreMath::Vector4((CoreMath::Scalar) evt.getPosition().x, (CoreMath::Scalar) evt.getPosition().y);
          m_pSelectedPoint = &*it;
        }
      }
      m_bMouseHeld = true;
    ...
    else
    {
      if(m_pSelectedPoint != NULL)
        *m_pSelectedPoint = CoreMath::Vector4((CoreMath::Scalar) evt.getPosition().x, (CoreMath::Scalar) evt.getPosition().y);
    }
  }
